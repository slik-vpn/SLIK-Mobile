...
def check_multiservice_crm(bot_text: str) -> None:
    ...
    status_filter_block = function_block(bot_text, "orders_filter_keyboard")
    filter_text_block = function_block(bot_text, "orders_filter_text")
    order_keyboard_block = function_block(bot_text, "order_card_keyboard")
    order_card_block = function_block(bot_text, "build_order_card_text")
    record("admin order status chooser contains only order status buttons", all(x in status_filter_block for x in ("🆕 Новые", "⏳ Ожидают оплаты", "✅ Оплаченные", "🤖 Автовыдача", "⏳ Ожидают выдачи", "📤 Выданные / выполненные", "⚠️ Требуют внимания", "❌ Отменённые", "↩️ Возвраты")) and "🍎 Apple ID" not in status_filter_block.split('"product"', 1)[0])
    record("choose status wording is not used for product payment or fulfillment filters", "Выберите статус заказа" in filter_text_block and "Выберите товарную категорию" in filter_text_block and "Выберите состояние оплаты" in filter_text_block and "Выберите состояние выдачи" in filter_text_block)
    ...
    record("manual payment confirmation sets payment status paid", 'payment_status="paid"' in payment_confirm_block and "paid_at=paid_at" in payment_confirm_block and "payment_paid_at=" in payment_confirm_block)
...
def check_bot_contract(bot_text: str, env_example_text: str) -> None:
    ...
    record("Apple ID empty cards stays waiting and keeps supplier order", "code_pending" in apple_fulfill_block and "waiting_supplier_code" in maybe_fulfill_block and "paid_waiting_supplier_code" in maybe_fulfill_block and "Код ещё готовится" in maybe_fulfill_block and "supplier_order_id" in maybe_fulfill_block)
    record("Apple ID delayed code retry uses GET only after supplier order", "fetch_fazercards_order_details(existing_supplier_order_id)" in apple_fulfill_block and "fetch_fazercards_order_details(supplier_order_id)" in apple_fulfill_block and "fazercards_post_order" not in details_block and "client.get" in details_block)
    record("Apple ID retry can issue delayed string card and mark stock used", 'if fields.get("giftcard_code")' in apple_fulfill_block and 'status="used"' in apple_fulfill_block and '"fulfillment_status": "issued"' in maybe_fulfill_block and "delivery_code" in upsert_stock_block and '"status": "used" if (used_status_locked or code) else status' in upsert_stock_block)
    record("Apple ID retry exhaustion goes to manual review without repeat POST", "code_pending_exhausted" in maybe_fulfill_block and "Код пока не получен от поставщика. Проверьте заказ у поставщика вручную." in maybe_fulfill_block and 'elif category == "apple_id" and (result.get("supplier_purchase_attempted")' in maybe_fulfill_block)
    record("Apple ID response shape has only keys and types", "def supplier_response_shape" in bot_text and " keys:" in function_block(bot_text, "supplier_response_shape") and " type:" in function_block(bot_text, "supplier_response_shape") and "mask_giftcard_code" in function_block(bot_text, "sanitized_supplier_response"))
    ...
    record("Apple ID details stock upsert marks used or pending without duplicates", "def upsert_apple_id_fazercards_stock_item" in bot_text and "supplier_order_id" in upsert_stock_block and "normalize_stock_code" in upsert_stock_block and '"status": "used" if (used_status_locked or code) else status' in upsert_stock_block and "upsert_apple_id_fazercards_stock_item(order, existing_supplier_order_id" in apple_fulfill_block and "upsert_apple_id_fazercards_stock_item(order, supplier_order_id" in apple_fulfill_block)
...
def check_apple_id_rub_market_pricing(bot_text: str) -> None:
    ...
    record("FazerCards stock used supplier_order_id never becomes available", 'used_status_locked = stock_item_is_used(target)' in stock_upsert_block and 'if used_status_locked:' in stock_upsert_block and 'new_status = current_status' in stock_upsert_block and 'new_status = "available"' in stock_upsert_block and stock_upsert_block.find('if used_status_locked:') < stock_upsert_block.find('new_status = "available"'))
    record("FazerCards stock used supplier_order_id never becomes pending", 'used_status_locked = stock_item_is_used(target)' in stock_upsert_block and 'if used_status_locked:' in stock_upsert_block and 'new_status = current_status' in stock_upsert_block and 'new_status = "pending"' in stock_upsert_block and stock_upsert_block.find('if used_status_locked:') < stock_upsert_block.find('new_status = "pending"'))
    ...
    stock_status_block = function_block(bot_text, "stock_status_label")
    stock_mark_done_block = function_block(bot_text, "stock_mark_done_label")
    record("stock_status_label accepts category", 'category: str = ""' in stock_status_block)
    record("apple_id pending status says waits for code", "⏳ Ожидает код" in stock_status_block and "⏳ Ожидают код" in stock_status_block)
    record("telegram_stars pending status says waits for fulfillment", "telegram_stars" in stock_status_block and "⏳ Ожидает выполнения" in stock_status_block and "⏳ Ожидают выполнения" in stock_status_block)
    record("telegram_premium pending status says waits for fulfillment", "telegram_premium" in stock_status_block and "⏳ Ожидает выполнения" in stock_status_block and "⏳ Ожидают выполнения" in stock_status_block)
    record("telegram_stars used status says completed", "telegram_stars" in stock_status_block and "📤 Выполнена" in stock_status_block and "📤 Выполненные" in stock_status_block)
    record("telegram_premium used status says completed", "telegram_premium" in stock_status_block and "📤 Выполнена" in stock_status_block and "📤 Выполненные" in stock_status_block)
    stock_summary_block = function_block(bot_text, "stock_item_summary_text")
    stock_display_block = function_block(bot_text, "stock_display_id")
    find_stock_block = function_block(bot_text, "find_stock_item")
    stock_mark_button_block = function_block(bot_text, "stock_mark_issued_button_label")
    telegram_ui_blocks = "\n".join([stock_status_block, stock_category_text_block, stock_category_keyboard_block, function_block(bot_text, "stock_list_text"), stock_summary_block, stock_mark_button_block, function_block(bot_text, "stock_mark_issued_confirm_text"), stock_mark_done_block])
    record("Telegram stock UI does not hard-code waits for code", "Ожидают код" not in telegram_ui_blocks.replace("⏳ Ожидают код", "") and "ищет код" not in telegram_ui_blocks)
    record("Telegram stock UI does not hard-code issued plural", "Выданные" not in telegram_ui_blocks.replace("📤 Выданные", ""))
    record("Telegram mark action is completed", "def stock_mark_done_label" in bot_text and "telegram_stars" in stock_mark_done_block and "telegram_premium" in stock_mark_done_block and "📤 Отметить как выполненную" in stock_mark_done_block)
    record("Apple ID mark action remains issued", "def stock_mark_done_label" in bot_text and "📤 Отметить как выданную" in stock_mark_done_block)
    record("manual stock issued button exists without technical used label", "Пометить used" not in bot_text and "admin_stock_mark_issued:" in bot_text and "admin_stock_mark_issued_confirm:" in bot_text)
    record("stock_display_id exists and prefers supplier order", "def stock_display_id(item: dict) -> str" in bot_text and 'item.get("supplier_order_id")' in stock_display_block and "return supplier_order_id" in stock_display_block)
    record("stock_display_id falls back to SLIK order, product title, then internal id", "Заказ SLIK #" in stock_display_block and "stock_item_product_label(item)" in stock_display_block and 'item.get("id")' in stock_display_block)
    record("stock summary uses position label instead of technical main id", "📦 Позиция:" in stock_summary_block and "stock_display_id(item)" in stock_summary_block and "ID позиции" not in stock_summary_block)
    record("stock summary does not show stock_ as main id when supplier order exists", "supplier_order_id" in stock_summary_block and "Внутренний ID" in bot_text and 'not item.get("supplier_order_id")' in stock_summary_block)
    record("stock mark issued button shows display id while callback keeps internal id", "def stock_mark_issued_button_label" in bot_text and "stock_display_id(item)" in stock_mark_button_block and "admin_stock_mark_issued:{stock_id}" in stock_list_keyboard_block)
    record("stock search accepts supplier order, internal id and normalized codes", 'item.get("id")' in find_stock_block and 'item.get("supplier_order_id")' in find_stock_block and 'normalize_stock_code(item.get("delivery_code"))' in find_stock_block and 'normalize_stock_code(item.get("giftcard_code"))' in find_stock_block)
    record("stock search prompt uses readable position wording", "Отправьте номер позиции или заказа у поставщика" in bot_text and "Отправьте ID позиции" not in bot_text)
    ...
    record("stock list uses product field labels", "Внутренний ID" in bot_text and "Заказ у поставщика" in bot_text and "Диагностика ответа поставщика" in bot_text and "response_shape:" not in function_block(bot_text, "stock_list_text"))
    ...
    apple_upsert_block = function_block(bot_text, "upsert_apple_id_fazercards_stock_item")
    diagnostics_block = function_block(bot_text, "run_stock_persistence_diagnostics")
    report_block = function_block(bot_text, "fazercards_stock_sync_report_text")
    record("manual mark issued persists after reload", "save_stock(items)" in mark_issued_block and "load_stock()" in mark_issued_block and 'return normalize_stock_item(saved), "marked_issued"' in mark_issued_block and '"save_failed"' in mark_issued_block)
    record("used stock item with same supplier_order_id remains used after Apple ID upsert", "stock_item_is_used(target)" in apple_upsert_block and '"status": "used" if (used_status_locked or code) else status' in apple_upsert_block and "restore_used_stock_fields(target, preserved_used)" in apple_upsert_block)
    record("Apple ID supplier details upsert with code marks item used not available", '"status": "used" if (used_status_locked or code) else status' in apple_upsert_block and 'target["used_order_id"] = str(order.get("id")' in apple_upsert_block and '"available" if code' not in apple_upsert_block)
    record("used stock item with same code remains used after upsert", 'normalize_stock_code(item.get("delivery_code") or item.get("giftcard_code")) == code_key' in stock_upsert_block and "stock_item_is_used(target)" in stock_upsert_block and "restore_used_stock_fields(target, preserved_used)" in stock_upsert_block)
    record("upsert does not create duplicate for already used item", "existing = item" in stock_upsert_block and "target = existing or" in stock_upsert_block and "items.append(target)" in stock_upsert_block and stock_upsert_block.find("target = existing or") < stock_upsert_block.find("items.append(target)"))
    record("sync/upsert does not return used item to available or pending", "if used_status_locked:" in stock_upsert_block and "new_status = current_status" in stock_upsert_block and "restore_used_stock_fields(target, preserved_used)" in stock_upsert_block)
    record("Telegram Stars supplier completed maps to local used", "telegram_service_completed" in stock_upsert_block and 'category in {"telegram_stars", "telegram_premium"}' in stock_upsert_block and 'new_status = "used"' in stock_upsert_block and 'target["comment"] = "Выполнено поставщиком"' in stock_upsert_block)
    record("Telegram Premium supplier completed maps to local used", "telegram_service_completed" in stock_upsert_block and '"fulfilled"' in stock_upsert_block and '"delivered"' in stock_upsert_block and 'target["used_at"] = str(completed_at or now_str())' in stock_upsert_block)
    record("Telegram completed sync/upsert stores source order", 'source_order_id = str(_first_order_value(order_data' in stock_upsert_block and 'target["used_order_id"] = source_order_id' in stock_upsert_block)
    telegram_stars_fulfill_block = function_block(bot_text, "auto_fulfill_telegram_stars_order")
    telegram_premium_fulfill_block = function_block(bot_text, "auto_fulfill_telegram_premium_order")
    record("Telegram auto fulfillment upserts supplier response", "upsert_stock_item_from_fazercards_order(data) if supplier_order_id" in telegram_stars_fulfill_block and "upsert_stock_item_from_fazercards_order(data) if supplier_order_id" in telegram_premium_fulfill_block)
    record("Telegram used service item is not returned back to pending", "telegram_service_completed" in stock_upsert_block and "if used_status_locked:" in stock_upsert_block and "new_status = current_status" in stock_upsert_block and stock_upsert_block.find("if used_status_locked:") < stock_upsert_block.find('new_status = "used"') and "restore_used_stock_fields(target, preserved_used)" in stock_upsert_block)
    record("Telegram completed UI shows completed labels", "📤 Выполнена" in stock_status_block and "📤 Выполненные" in stock_status_block and "⏳ Ожидает выполнения" in stock_status_block)
    record("full gift card codes do not appear in sync reports/log-like text", "mask_giftcard_code" in report_block and "giftcard_code" not in function_block(bot_text, "stock_persistence_diagnostics_text") and "SLIK-DUMMY-CODE-0000" not in function_block(bot_text, "stock_persistence_diagnostics_text"))
    record("stock diagnostics cleanup removes only its own temporary item", "current_items = load_stock()" in diagnostics_block and "original_items" not in diagnostics_block.split("finally:", 1)[-1] and "cleanup_code_key" in diagnostics_block)
    record("stock diagnostics button exists and avoids supplier POST", "🧪 Проверить сохранение склада" in bot_text and "admin_stock_diagnostics" in bot_text and "fazercards_post_order" not in diagnostics_block and "client.post" not in diagnostics_block)
    ...
    manual_confirm_block = function_block(bot_text, "handle_order_payment_confirm")
    ...
    record("payment chat gets manual confirmation events", "manual_payment_confirmed" in manual_confirm_block and "manual_confirm" in manual_confirm_block)
...
def check_fazercards_stock_sync(bot_text: str, config_example_text: str) -> None:
    ...
    record("FazerCards order upsert exists and preserves used/invalid", "def upsert_stock_item_from_fazercards_order" in bot_text and "supplier_order_id" in upsert_block and 'used_status_locked = stock_item_is_used(target)' in upsert_block and 'current_status == "invalid"' in upsert_block)
    ...
    record("FazerCards failed statuses invalidate pending only", "supplier_failed_statuses" in upsert_block and 'new_status = "invalid"' in upsert_block and 'if used_status_locked:' in upsert_block and upsert_block.find('if used_status_locked:') < upsert_block.find('new_status = "invalid"'))
    ...
    delivery_block = function_block(bot_text, "send_apple_id_code_to_client")
    maybe_block = function_block(bot_text, "maybe_auto_fulfill_paid_order")
    keyboard_block = function_block(bot_text, "order_card_keyboard")
    callback_block = function_block(bot_text, "handle_callback")
    record("Apple ID client delivery is confirmed before issued", "async def send_apple_id_code_to_client" in bot_text and 'client_delivery_status": "sent"' in delivery_block and 'fulfillment_status": "issued"' in delivery_block and 'client_delivery_message_id' in delivery_block)
    record("Apple ID send failure keeps code and marks delivery failed", 'client_delivery_status": "failed"' in delivery_block and 'fulfillment_status": "client_delivery_failed"' in delivery_block and 'paid_code_received_delivery_failed' in delivery_block and 'Код получен, но не отправлен клиенту' in delivery_block)
    record("Apple ID auto fulfillment sends saved code without repeat purchase", 'order.get("giftcard_code") and order.get("client_delivery_status") != "sent"' in bot_text and 'saved_code' in bot_text and 'await send_apple_id_code_to_client(context, updated)' in maybe_block)
    record("Apple ID admin resend button exists and is owner/admin guarded", "📨 Отправить код клиенту повторно" in keyboard_block and "order_resend_code:" in callback_block and "has_admin_access(query.from_user) or has_owner_access(query.from_user)" in callback_block and "send_apple_id_code_to_client(context, order)" in callback_block)
    record("Apple ID delivery failure notifications mask full codes", "⚠️ Код Apple ID получен, но не отправлен клиенту" in bot_text and "mask_giftcard_code" in function_block(bot_text, "auto_fulfillment_admin_text") and "Apple ID client delivery retry failed" in callback_block)

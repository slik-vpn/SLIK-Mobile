...
def check_apple_id_rub_market_pricing(bot_text: str) -> None:
    ...
    record("Telegram sync report shows endpoint samples", all(x in telegram_report_block for x in ("Stars endpoint", "Premium endpoint", "Samples Stars", "Samples Premium", "price_usd", "stock")))
    ...
    record("Telegram sync report has official endpoint diagnostic fields", all(x in telegram_sync_block and x in telegram_report_block for x in ("telegram_stars_endpoint_ok", "telegram_premium_endpoint_ok", "stars_items_scanned", "premium_items_scanned", "stars_candidates_found", "premium_candidates_found", "stars_samples", "premium_samples", "fallback_used")))
    record("Telegram raw diagnostics are shown in sync report", "telegram_raw_diagnostics" in bot_text and all(x in telegram_report_block for x in ("raw type", "raw keys", "raw message", "sample raw", "raw_sample")))
    ...
    record("Telegram quote mode supports standard Stars packages", "TELEGRAM_STARS_STANDARD_PACKAGES = (50, 100, 200, 250, 500, 750, 1000, 1500" in bot_text and "stars_quotes_requested" in telegram_sync_block and "_telegram_quote_supplier_position" in telegram_sync_block)
    ...
    record("Telegram quote username setting exists and is reported when missing", "quote_test_username" in bot_text and "TELEGRAM_QUOTE_USERNAME_INPUT" in bot_text and "Для Telegram Stars quote нужен test username" in telegram_sync_block and "Quote test username" in telegram_report_block)

...
def check_bot_contract(bot_text: str, env_example_text: str) -> None:
    ...
    record("Apple ID tech chat shows response shape without full code", "region:" in notify_auto_block and "currency:" in notify_auto_block and "FazerCards Apple ID details fetched but code missing" in notify_auto_block and "response_shape:" in notify_auto_block and "giftcard_code" not in notify_auto_block)
...
def check_fazercards_stock_sync(bot_text: str, config_example_text: str) -> None:
    ...
    record("FazerCards pending response_shape stores only keys", "supplier_response_shape(order_data)" in upsert_block and "last_response_shape" in upsert_block and "response_shape:" in stock_list_block)
    ...
    record("FazerCards full codes are masked in chats/reports", "mask_giftcard_code" in sanitized_block and "sensitive_container_keys" in sanitized_block and "mask_giftcard_code" in report_block and "response_shape:" in stock_list_block and "str(current[0])" not in shape_block)

...
def check_bot_contract(bot_text: str, env_example_text: str) -> None:
    ...
    record("Stars fulfillment statuses are saved", '"fulfillment_status": "issued"' in maybe_fulfill_block and '"status": "issued"' in maybe_fulfill_block and 'status=("paid_waiting_manual_issue" if status == "manual_required" else status)' in maybe_fulfill_block and "stars_amount" in maybe_fulfill_block)
...
def check_telegram_premium_auto_fulfillment(bot_text: str) -> None:
    ...
    record("Premium supplier payload includes product/card id duration and recipient", all(x in premium_helper for x in ("product_id", "card_id", "duration_months", "months", "recipient", "username")))

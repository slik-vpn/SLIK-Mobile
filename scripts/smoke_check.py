...
def check_apple_id_rub_market_pricing(bot_text: str) -> None:
    ...
    record("market sources include diagnostics", "default_market_source_diagnostics" in bot_text and '"diagnostics": default_market_source_diagnostics()' in bot_text)
    record("market sources include search_url_template", '"search_url_template": "https://plati.market/search/{query}"' in bot_text and '"search_url_template": "https://ggsel.net/search/{query}"' in bot_text)
    ...
    public_items_block = function_block(bot_text, "extract_public_market_items")
    calc_block = function_block(bot_text, "calculate_apple_id_recommended_price")
    refresh_block = callback_branch_block(bot_text, 'elif data.startswith("admin_apple_id_pricing_refresh:")')
    debug_block = callback_branch_block(bot_text, 'elif data.startswith("admin_apple_id_pricing_debug:")')
    ...
    api_extract_block = function_block(bot_text, "extract_multitransfer_api_candidates")
    api_try_block = function_block(bot_text, "try_multitransfer_api_candidates")
    chunk_extract_block = function_block(bot_text, "extract_nextjs_chunk_candidates")
    chunk_scan_block = function_block(bot_text, "scan_multitransfer_js_chunks_for_price_sources")
    build_id_block = function_block(bot_text, "extract_nextjs_build_id")
    safe_get_block = function_block(bot_text, "multitransfer_url_is_safe_get")
    display_title_block = function_block(bot_text, "apple_id_display_title_with_nominal")
    record("Ozon exact price is tied to exact nominal fragment", "exact_fragments" in ozon_block and "extract_rub_prices_from_fragment(fragment)" in ozon_block and "script_market_fragments" in ozon_block and "exact_price_not_found" in ozon_block)
    record("Ozon diagnostics include scripts and API candidates", "api_url_candidates" in bot_text and "scripts_found" in bot_text and "json_scripts_found" in bot_text)
    record("Ozon diagnostics include Next.js buckets", "js_chunk_candidates" in bot_text and "next_data_candidates" in bot_text and "static_skipped_count" in bot_text)
    record("Ozon diagnostics include mention counters", "currency_mentions" in bot_text and "rub_mentions" in bot_text and "nominal_mentions" in bot_text and "region_mentions" in bot_text)
    record("extract_multitransfer_api_candidates exists", "def extract_multitransfer_api_candidates" in bot_text and "fetch" in api_extract_block and "axios" in api_extract_block and "XMLHttpRequest" in api_extract_block)
    record("extract_multitransfer_api_candidates skips static media/fonts", "multitransfer_url_is_static_asset" in api_extract_block and ".woff2" in bot_text and ".otf" in bot_text and ".ttf" in bot_text and "/_next/static/media/" in bot_text)
    record("extract_nextjs_chunk_candidates exists", "def extract_nextjs_chunk_candidates" in bot_text and "_next/static/chunks" in chunk_extract_block and ".js" in chunk_extract_block)
    record("scan_multitransfer_js_chunks_for_price_sources exists", "async def scan_multitransfer_js_chunks_for_price_sources" in bot_text and "client.get" in chunk_scan_block and "client.post" not in chunk_scan_block)
    record("extract_nextjs_build_id exists", "def extract_nextjs_build_id" in bot_text and "__NEXT_DATA__" in build_id_block and "buildId" in build_id_block)
    record("try_multitransfer_api_candidates exists", "async def try_multitransfer_api_candidates" in bot_text)
    record("API candidates use only GET", "client.get" in api_try_block and "client.post" not in api_try_block and "POST" not in api_try_block)
    record("safe GET skips unsafe endpoints", all(word in safe_get_block for word in ("purchase", "payment", "create", "checkout", "pay", "order")) and "giftcards" in safe_get_block)
    record("safe GET allows only API/data endpoints", '"/api/"' in safe_get_block and '"/_next/data/"' in safe_get_block and '"/ozon/_next/data/"' in safe_get_block)
    record("no forbidden giftcards order endpoint", "/giftcards/order" not in bot_text)
    record("no client.post in Ozon diagnostics", "client.post" not in ozon_block and "client.post" not in api_try_block and "client.post" not in chunk_scan_block)
    record("dynamic_price_api_not_found is handled safely", "dynamic_price_api_not_found" in bot_text and "dynamic_price_api_not_found" in ozon_block)
    record("dynamic_price_source_not_found is handled safely", "dynamic_price_source_not_found" in bot_text and "dynamic_price_source_not_found" in ozon_block)
    record("Apple ID display title does not duplicate nominal", "def apple_id_display_title_with_nominal" in bot_text and "title_has_nominal" in display_title_block and "apple_id_display_title_with_nominal(product)" in bot_text)
    record("Plati/GGSEL do not collect all prices after one page exact match", "apple_id_exact_market_match(html, product)" not in public_block and "for fragment in fragments" in public_items_block and "extract_rub_prices_from_fragment(fragment)" in public_items_block)
    ...
    record("source error does not break pricing screen", "normalize_market_source_diagnostics(src.get(\"diagnostics\"))" in function_block(bot_text, "apple_id_pricing_text"))
    record("debug mode does not change price_rub", "admin_apple_id_pricing_debug" in bot_text and "set_apple_id_product" not in debug_block and "price_rub" not in debug_block)
    record("refresh sources do not change price_rub", "update_apple_id_market_source" in refresh_block and "price_rub" not in refresh_block)

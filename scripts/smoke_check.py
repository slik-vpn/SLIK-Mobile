...
def check_apple_id_rub_market_pricing(bot_text: str) -> None:
    ...
    record("Telegram Stars price_per_star report fields exist", all(x in telegram_report_block for x in ("Stars price per star", "Stars min amount", "Stars max amount", "Stars packages generated")))
    record("Telegram Stars price_per_star sync does not require username", "Stars requires params:" in telegram_report_block and 'report["stars_requires_params"] = "no"' in telegram_sync_block and "Для Telegram Stars quote нужен test username" in telegram_sync_block)

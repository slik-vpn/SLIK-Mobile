...
def check_bot_contract(bot_text: str, env_example_text: str) -> None:
    ...
    record("Apple ID details stock upsert marks used or pending without duplicates", "def upsert_apple_id_fazercards_stock_item" in bot_text and "supplier_order_id" in upsert_stock_block and "normalize_stock_code" in upsert_stock_block and '"status": "used" if used_status_locked else ("available" if code else status)' in upsert_stock_block and "upsert_apple_id_fazercards_stock_item(order, existing_supplier_order_id" in apple_fulfill_block and "upsert_apple_id_fazercards_stock_item(order, supplier_order_id" in apple_fulfill_block)
...
def check_apple_id_rub_market_pricing(bot_text: str) -> None:
    ...
    record("used stock item with same supplier_order_id remains used after Apple ID upsert", "stock_item_is_used(target)" in apple_upsert_block and '"status": "used" if used_status_locked' in apple_upsert_block and "restore_used_stock_fields(target, preserved_used)" in apple_upsert_block)

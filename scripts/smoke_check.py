...
def check_bot_contract(bot_text: str, env_example_text: str) -> None:
    ...
    record("Apple ID stock admin add flow exists", "🍎 Склад Apple ID" in bot_text and "APPLE_ID_STOCK_INPUT_ADD" in bot_text and "admin_apple_id_stock_add" in bot_text and "supplier_order_id | region | amount | currency | code" in stock_input_block)
    record("Apple ID stock add checks access duplicate and available status", "has_admin_access(update.effective_user)" in function_block(bot_text, "handle_client_crm_input") and "apple_id_stock_code_exists" in function_block(bot_text, "add_apple_id_stock_code") and '"status": "available"' in function_block(bot_text, "add_apple_id_stock_code"))
    ...
    record("Apple ID missing-code diagnostics include safe response shape", "def supplier_response_shape" in bot_text and "giftcard_code_missing: response_keys=" in apple_fulfill_block and "supplier_order_id present but order details endpoint not configured" in apple_fulfill_block)
    record("Apple ID tech chat shows gift-card fields without full code", "region:" in notify_auto_block and "currency:" in notify_auto_block and "FazerCards Apple ID fulfillment failed" in notify_auto_block and "giftcard_code" not in notify_auto_block)
    ...
    record("Apple ID retry with supplier order uses read-only details first", "async def fetch_fazercards_order_details" in bot_text and "client.get" in function_block(bot_text, "fetch_fazercards_order_details") and "apple_id_can_fetch_existing_supplier_order" in retry_branch_block and "fazercards_post_order" not in function_block(bot_text, "fetch_fazercards_order_details"))
    record(
...
def check_telegram_premium_auto_fulfillment(bot_text: str) -> None:
    ...
    record("CRM card shows Premium supplier and fulfillment fields", all(x in order_card_block for x in ("Категория", "Товар", "Получатель", "Статус выдачи", "Поставщик", "Supplier order ID")))

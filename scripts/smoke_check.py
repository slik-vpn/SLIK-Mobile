...
def check_apple_id_rub_market_pricing(bot_text: str) -> None:
    ...
    record("Telegram read-only helpers and official endpoints exist", "async def fetch_fazercards_telegram_stars_readonly" in bot_text and "async def fetch_fazercards_telegram_premium_readonly" in bot_text and "/telegram/stars" in bot_text and "/telegram/premium" in bot_text)
    ...
    record("Telegram sync avoids buy/order POST endpoints", all(x not in bot_text for x in ("/api/v2/telegram/stars/buy", "/api/v2/telegram/premium/buy", "/giftcards/order")) and "client.post" not in telegram_sync_block)

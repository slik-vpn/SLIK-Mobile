...
В кабинете клиент видит имя, Telegram ID, статус, количество активных заявок, сумму покупок, `SLIK Balance` в долларах и количество приглашённых. Статус пересчитывается по `total_spent`: `Traveller` до $10, `Explorer` от $10, `Pro` от $50 и `VIP` от $100. Кнопка `📦 Мои заказы` показывает заказы текущего пользователя из `orders.json`; если заказов нет, бот отвечает: `У вас пока нет заказов.`

Кнопка `👥 Пригласить друга` показывает персональную ссылку вида `https://t.me/<bot>?start=ref_<telegram_id>`. При входе по `/start ref_<id>` бот сохраняет пригласившего, а после первой заявки приглашённого начисляет по `$1` на SLIK Balance другу и пригласившему. Кнопка `💰 SLIK Balance` показывает текущий SLIK Balance, количество приглашённых и сколько реферальных бонусов уже начислено.

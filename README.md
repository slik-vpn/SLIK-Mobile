...
В админ-панели доступен экран `💱 Курс USD/RUB`. На нём администратор может проверить текущий рыночный курс, задать ручной курс, сбросить ручной курс, настроить процент наценки и сбросить наценку к значению `USD_RUB_MARKUP_PERCENT`. Настройки сохраняются в `bot/config.json`: `rate_checked_at`, `markup_percent`, `final_usd_rub_rate`, а также последний рыночный курс и источник проверки.
...
2. Fallback из `.env`: `ORDERS_CHAT_ID`, `CLIENT_ACTIVITY_CHAT_ID`, `NEW_CLIENTS_CHAT_ID`, `PAYMENTS_CHAT_ID`, `TECH_ALERTS_CHAT_ID`, `RATE_CHAT_ID`.

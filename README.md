...
| `MANAGER` | Все функции `USER` плюс доступ к админ-панели, заказам, CRM-заготовкам клиентов/новостей и бэкапам. Список менеджеров подготовлен в `config.json` через поле `managers`. |
| `ADMIN` | Все функции `MANAGER` плюс текущие административные команды из списка `admins` в `config.json`. |

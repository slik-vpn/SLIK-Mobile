...
Кнопка `📦 Мои заказы` показывает аккуратную историю заказов текущего пользователя из `orders.json`: сверху выводятся общее количество заказов и потраченная сумма в USD, ниже — последние 10 заказов с разделителями, номером, страной, тарифом, суммой, датой и понятным статусом (`Новый`, `В работе`, `Выдан`, `Отменён`). Каждый заказ можно открыть в безопасной карточке только владельцу заказа. Кнопка `🔁 Повторить заказ` позволяет пользователю быстро оформить новый заказ на основе старого тарифа: бот создаёт новый checkout order, старый заказ не меняет, а если тариф больше недоступен — предлагает выбрать актуальный пакет. Если заказов больше 10, бот добавляет подпись `Показаны последние 10 заказов.`; если заказов нет, отвечает: `📦 У вас пока нет заказов.`

Cashback временно отключён feature flag-переменной `CASHBACK_ENABLED=false` (значение по умолчанию). При выключенном флаге бот не начисляет cashback, не меняет `SLIK Balance`, не выставляет `cashback_awarded: true` и не отправляет уведомление о начислении. Для будущего включения нужно явно задать `CASHBACK_ENABLED=true`. Логика cashback остаётся в проекте: после включения успешная заявка начисляет cashback на `SLIK Balance` в USD по текущему статусу клиента: 1% для Traveller, 2% для Explorer, 3% для Nomad, 5% для Premium и 7% для Ambassador.
...
### Проверка курса USD/RUB

В админке доступен экран **Админ-панель → 💳 Платёжные способы → 💱 Курс USD/RUB**. Он показывает текущий курс бота, источник (`manual`, Яндекс, ЦБ РФ, внешний API или fallback `USD_RUB_FALLBACK_RATE`), время проверки, наценку к курсу, итоговый курс с наценкой и примеры расчёта для `$10`, `$20` и `$50`. Кнопка **🔄 Проверить сейчас** заново вызывает текущую функцию получения курса и не создаёт заказ.

Ручной курс можно установить кнопкой **✏️ Ручной курс** или командой `/set_usd_rate 91.50`; значение сохраняется в `config.json` как `payment.manual_usd_rub_rate`, имеет приоритет над автоматическими источниками, а `rate_source` становится `manual`. Кнопка **🧹 Сбросить ручной курс** удаляет ручное значение и возвращает автоисточник/fallback.

Наценка настраивается кнопкой **✏️ Изменить наценку** или командой `/set_rate_markup 5`; значение сохраняется в `config.json` как `payment.usd_rub_markup_percent`. Приоритет наценки: `config.json` → `.env` `USD_RUB_MARKUP_PERCENT` → default `0%`. Кнопка **🧹 Сбросить наценку** возвращает значение из `.env` или default. При создании оплаты картой в заказе фиксируются базовый `usd_rub_rate`, `rate_source`, `rate_checked_at`, `markup_percent`, `final_usd_rub_rate` и итоговый `rub_amount`. Клиент видит только итоговую сумму в ₽ и сообщение, что курс зафиксирован на момент создания заявки.
...
eSIM expiry reminder — мягкое напоминание пользователю, что выданная eSIM скоро закончится, с кнопками быстрого повторения заказа, выбора другой eSIM и поддержки. Дата окончания считается приблизительно от `issued_at` + срок тарифа из поля `days`; если срок распарсить нельзя, напоминание не отправляется. При переводе заказа в статус `issued` бот сохраняет `issued_at` и `expiry_reminder_sent: false`, если этих полей ещё нет. Напоминание отправляется только один раз, только для заказов в статусе `issued` и только для достаточно свежих заказов, чтобы после рестарта не было массовой рассылки. Настройки `.env`: `ESIM_EXPIRY_REMINDER_DAYS_BEFORE` — за сколько дней до окончания напоминать (по умолчанию `1`), `ESIM_EXPIRY_REMINDER_MAX_AGE_DAYS` — максимальный возраст выданного заказа для проверки (по умолчанию `60`).
...
Админка поддерживает отдельный раздел **🔔 Чаты уведомлений** для маршрутизации уведомлений Telegram-бота без ручного редактирования `.env` на сервере.
...
Короткая инструкция доступна прямо в боте: **Админ-панель → 🔔 Чаты уведомлений → 📘 Инструкция**.

Команда `/notification_routes` показывает текущие реальные маршруты уведомлений с `chat_id` и источником (`config`, `env` или `fallback ADMIN_CHAT_ID`) для заказов, действий клиентов, новых клиентов, оплат и технических уведомлений.

Как подключить групповой чат:

1. Добавьте бота в нужную Telegram-группу.
2. В этой группе отправьте команду `/chatid`.
3. Скопируйте ответ `Chat ID: <id>`.
4. Откройте **Админ-панель → 🔔 Чаты уведомлений**, выберите тип уведомлений и нажмите **✏️ Изменить**.
5. Вставьте `chat_id` числом, например `-1001234567890`, и проверьте отправку кнопкой **🧪 Тест**.

Команда `/chatid` показывает только ID текущего чата: в группе — ID группы, в личке — ID личного чата. Чтобы бот мог отправлять уведомления в группу, он должен быть добавлен в эту группу.

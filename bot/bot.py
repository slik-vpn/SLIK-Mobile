...
def is_telegram_supplier_category(text: str) -> bool:
    low = str(text or "").lower().replace("ё", "е")
    has_telegram = bool(re.search(r"\btelegram\b|\btg\b|телеграм", low))
    has_product_hint = bool(re.search(r"\bstars?\b|звезд|⭐|\bxtr\b|\bpremium\b|премиум", low))
    return has_telegram or (has_telegram and has_product_hint)


def fazercards_cards_payload_has_container(payload: dict) -> bool:
    if not isinstance(payload, dict):
        return False
    for field in ("offers", "items", "cards", "data", "products"):
        if isinstance(payload.get(field), list):
            return True
    for parent_field in ("data", "result"):
        parent = payload.get(parent_field)
        if isinstance(parent, dict):
            for field in ("offers", "items", "cards", "data", "products"):
                if isinstance(parent.get(field), list):
                    return True
    return False


def telegram_sync_report_text(report: dict) -> str:
    lines = [
        "🔗 <b>FazerCards Telegram sync</b>",
        "",
        f"Категорий проверено: {int(report.get('categories_scanned') or 0)}",
        f"Telegram категорий найдено: {int(report.get('telegram_categories_found') or 0)}",
        f"Карточек проверено: {int(report.get('cards_scanned') or 0)}",
        f"Stars найдено: {int(report.get('stars_candidates_found') or 0)}",
        f"Premium найдено: {int(report.get('premium_candidates_found') or 0)}",
        f"Pending: {int(report.get('pending') or 0)}",
        f"Not found: {int(report.get('not_found') or 0)}",
        f"Unsupported: {int(report.get('unsupported') or 0)}",
        f"Ошибки: {int(report.get('errors') or 0)}",
    ]
    samples = report.get("diagnostic_samples") if isinstance(report.get("diagnostic_samples"), list) else []
    if samples:
        lines.extend(["", "Диагностика:", *[f"• {html_escape(str(sample))}" for sample in samples[:5]]])
    if report.get("error"):
...
async def sync_telegram_fazercards_bulk(kind: str = "all") -> dict:
    report = {
        ...
        "telegram_categories_found": 0,
        "cards_scanned": 0,
        "stars_candidates_found": 0,
        "premium_candidates_found": 0,
        "pending_stars_count": 0,
        "pending_premium_count": 0,
        "diagnostic_samples": [],
        "lines": [],
    }
    products_payload = await fetch_fazercards_products_readonly()  # GET /giftcards
    ...
    supplier_read_success = True
    for category in items:
        if not isinstance(category, dict):
            continue
        report["categories_scanned"] += 1
        category_text = fazercards_product_value(category, "name") or str(category)
        if not is_telegram_supplier_category(category_text):
            continue
        report["telegram_categories_found"] += 1
        if len(report["diagnostic_samples"]) < 5:
            report["diagnostic_samples"].append(f"category: {category_text[:120]}")
        category_id = fazercards_category_id_value(category)
        payload = await fetch_fazercards_giftcards_cards_readonly(category_id)  # GET /giftcards/cards?category_id=...
        if not payload.get("ok"):
            report["errors"] += 1
            supplier_read_success = False
            continue
        cards = fazercards_cards_from_payload(payload)
        if not cards and not fazercards_cards_payload_has_container(payload):
            report["errors"] += 1
            supplier_read_success = False
            if len(report["diagnostic_samples"]) < 5:
                report["diagnostic_samples"].append(f"unrecognized cards payload: {category_text[:100]}")
            continue
        report["cards_scanned"] += len(cards)
        for card in cards:
            if len(report["diagnostic_samples"]) < 5:
                report["diagnostic_samples"].append(f"card: {(fazercards_product_value(card, 'name') or str(card))[:120]}")
    ...
    if kind in ("all", "stars"):
        ...
        cfg = load_config(); cfg["telegram_stars_pending_supplier_positions"] = pending[:100]; save_config(cfg)
        report["pending_stars_count"] = len(pending); report["pending"] += len(pending)
    ...
    if kind in ("all", "premium"):
        ...
        cfg = load_config(); cfg["telegram_premium_pending_supplier_positions"] = pending[:100]; save_config(cfg)
        report["pending_premium_count"] = len(pending); report["pending"] += len(pending)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith(owner_callbacks) and not has_owner_access(query.from_user):
        ...
        await edit_or_send(query, context, telegram_sync_report_text(report), admin_telegram_services_keyboard())

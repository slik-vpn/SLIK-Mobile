...
def is_visible_apple_id_product(product: dict, enabled_only: bool = False) -> bool:
    ...
    if enabled_only and not product.get("enabled", True):
        return False
    return is_valid_apple_id_nominal(
        str(product.get("region") or ""),
        str(product.get("currency") or ""),
        product.get("amount"),
    )
...
def build_grid_keyboard(buttons: list[InlineKeyboardButton], columns: int = 3) -> list[list[InlineKeyboardButton]]:
    safe_columns = max(1, int(columns or 1))
    return [buttons[i:i + safe_columns] for i in range(0, len(buttons), safe_columns)]


def apple_id_grid_columns(region: str, count: int = 0) -> int:
    if region == "US":
        return 3
    if region == "TR" and count > 4:
        return 3
    return 2
...
def apple_id_products_keyboard(region: str) -> InlineKeyboardMarkup:
    products = apple_id_products_by_region(region, enabled_only=True)
    buttons = []
    for product in products:
        amount = f"${product['amount']}" if product["currency"] == "USD" else f"{product['amount']}₺"
        price = format_apple_id_client_price(product)
        label = f"{amount} — {price}"
        if len(label) > 18:
            label = amount
        buttons.append(InlineKeyboardButton(label, callback_data=f"apple_id_product:{product['id']}"))
...
async def show_apple_id_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not product or not is_visible_apple_id_product(product, enabled_only=True):
...
def apple_id_admin_region_keyboard(region: str) -> InlineKeyboardMarkup:
    products = apple_id_products_by_region(region)
    buttons = [InlineKeyboardButton(apple_nominal_text(p), callback_data=f"admin_apple_id_product:{p['id']}") for p in products]
    rows = build_grid_keyboard(buttons, apple_id_grid_columns(region, len(products)))
